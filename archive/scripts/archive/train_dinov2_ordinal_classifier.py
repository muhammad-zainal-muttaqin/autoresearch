"""Train an ordinal DINOv2 crop classifier for B1 < B2 < B3 < B4."""
...
from stage2_models import (
    CLASS_NAMES,
    N_CLASSES,
    DINOv2OrdinalClassifier,
    build_train_transforms,
    build_val_transforms,
    classifier_logits_to_probs,
    coral_levels_from_labels,
)


CROP_DIR = Path("/workspace/autoresearch/Dataset-Crops")
SAVE_PATH = Path("/workspace/autoresearch/stage2_dinov2_ordinal_classifier.pth")
BATCH_SIZE = 32
NUM_EPOCHS = 50
LR = 1e-3
...
def ordinal_loss(logits, labels, class_weights):
    levels = coral_levels_from_labels(labels, N_CLASSES)
...
def train_epoch(model, loader, optimizer, class_weights):
    ...
    for images, labels in loader:
        ...
        optimizer.zero_grad()
        logits = model(images)
        loss = ordinal_loss(logits, labels, class_weights)
        loss.backward()
        optimizer.step()

        total_loss += loss.item() * images.size(0)
        probs = classifier_logits_to_probs("dinov2_coral", logits)
...
def main():
    ...
    print("\nBuilding ordinal DINOv2 classifier...")
    model = DINOv2OrdinalClassifier(
        n_classes=N_CLASSES,
    ...
    print(f"Ordinal DINOv2 classifier saved to {SAVE_PATH}")
